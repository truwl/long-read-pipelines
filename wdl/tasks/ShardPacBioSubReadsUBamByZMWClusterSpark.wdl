...
workflow ShardPacBioSubReadsUBamByZMWClusterSpark {
    ...
    output {
        File spark_sbi_index = ShardInOneVM.spark_sbi_index

        Array[File] split_bam = ShardInOneVM.split_bam
        File original_header_hd_line = ShardInOneVM.original_header_hd_line
    }
#    I have to comment out these code because WDL's support for outputs of if-else tasks calls is lacking, it interprets the output as optional, which is wrong.
...
# when one prefers to perform the index-creation and sharding in one go
task ShardInOneVM {
    input {
        File input_ubam
    ...
    String sbi_prefix = basename(input_ubam, ".bam")
    ...
    output {
        File spark_sbi_index = "~{sbi_prefix}.sbi"
    ...
    RuntimeAttr default_attr = object {
        ...
        preemptible_tries:  0,
        max_retries:        0,
        docker:             "shuangbroad/gatk:custom_2"
    }
    RuntimeAttr runtime_attr = select_first([runtime_attr_override, default_attr])
    runtime {
        cpu:                    select_first([runtime_attr.cpu_cores,         default_attr.cpu_cores])
